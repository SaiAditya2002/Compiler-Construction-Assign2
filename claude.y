...
var_decl: ID_list COLON type SEMICOLON {printf("var_decl 1\n");}
	  | ID COLON type SEMICOLON {printf("var_decl 1\n"); addToSymbolTable($1, "", 0, 1);}
        | ID COLON ARRAY OF LBRACK NUM PERIOD PERIOD NUM RBRACK OF type SEMICOLON
        {
        	printf("var_decl 2\n");
...
write_stmt: WRITE LPAREN str RPAREN SEMICOLON
          ...
          | WRITE LPAREN expr COMMA expr RPAREN SEMICOLON  // Added support for writing multiple expressions
          ;
...
term: term MUL factor | term DIV factor | term MOD factor  // Added support for modulo operator (%)
    | factor
    ;
